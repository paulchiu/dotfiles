...
2. **Block-anchored**: `page_id` + `selection_with_ellipsis`. Anchors to a specific block. Whether the UI also captures a sub-selection within that block depends on the block type, not on anchor tightness (see Block-Type Sub-Selection Behaviour).
...
5. Snippet length only affects **uniqueness** (the start+end combo must match the page exactly once). It does **not** drive sub-selection on paragraph or bullet blocks (see Block-Type Sub-Selection Behaviour).
...
## Block-Type Sub-Selection Behaviour
...
- Test data, 2026-05-06 against Notion page `3503c6719946806d9bfcffaafead7219`: three variants on the same paragraph block (long anchor with ~50 char start and ~22 char end, tight ~10-char-per-side anchor, and a tightest-feasible word-scoped anchor) all returned `context="block"` with no `text-context`. The Notion UI highlighted the whole paragraph in every case.

**Implication**: tightening the anchor on a paragraph or bullet block will not produce a tighter UI highlight; anchor tightness there only matters for uniqueness validation. Set the user's expectations up front: on prose blocks, a comment underlines the whole block in the UI regardless of how surgical the anchor looks. The comment still lands correctly; readers just see a wider visual scope.

**Verifying which behaviour you got**: in `notion-get-comments` output, look at the `<discussion>` element. A `text-context="<phrase>"` attribute means Notion captured a sub-selection. Its absence means block-level highlight only. For paragraph and bullet blocks that is normal, not a failure; do not retry to force it. The create call returns success either way; the difference only shows up here.
...
2. **Set expectations**. Before posting, tell the user which findings will land on prose blocks (whole-block highlighting) and which will share a thread through block-discussion merging.
3. **Test one anchor first**. Post the most-likely-tricky finding as a single test, then `notion-get-comments` (`include_all_blocks: true`) and confirm `context="block"` (not page-level) and that the discussion landed on the intended block. Presence or absence of `text-context` follows block type (see Block-Type Sub-Selection Behaviour); either is normal.
4. **Post the rest**. Parallel calls are fine. Each anchored comment that lands on a fresh block creates its own discussion; ones that share a block get bundled.
5. **Verify**. Final `notion-get-comments` (`include_all_blocks: true`). Tally discussions and per-thread comment counts. Report the breakdown to the user.
6. **Stop on failure**. If anchoring repeatedly fails (multiple-occurrence errors with no good unique anchor), stop and report the limitation. Do not fall back to page-level.
...
Confirm `context="block"` (never page-level) and that each discussion landed on the intended block. Treat `text-context` as informational, not pass/fail. Call out any thread-merging in the report.
