...
These nested skills are richer than the matching `references/` stub. When the task matches, Read the nested `SKILL.md` and follow it exactly.
...
├─ Stateful coordination/real-time → nested/durable-objects/ (deep-dive)
...
├─ Strongly-consistent per-entity state → nested/durable-objects/ (DO storage)
...
├─ Build stateful AI agents → nested/agents-sdk/ (deep-dive)
...
| Durable Objects | `nested/durable-objects/` (deep-dive) |
...
| Agents SDK | `nested/agents-sdk/` (deep-dive) |
...
| Product | Reference |
|---------|-----------|
| Wrangler | `nested/wrangler/` (deep-dive) |
...
| Sandbox | `nested/sandbox-sdk/` (deep-dive) |
...
| Product | Reference |
|---------|-----------|
| Email (Sending + Routing) | `nested/cloudflare-email-service/` (deep-dive) |
