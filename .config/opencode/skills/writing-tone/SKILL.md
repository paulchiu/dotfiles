...
Machine-generated prose has a recognisable fingerprint; strip it. This list adapts the prose-relevant tells from [Wikipedia: Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing) so the URL doesn't need re-checking each time. Several already have full rules above (linked); the rest are new here.

**Tone & substance**

- No puffery or promotional adjectives ('vibrant', 'nestled', 'rich tapestry', 'breathtaking', 'diverse array', 'seamless', 'robust'). Name the concrete thing instead.
- No significance-inflation: 'stands as a testament to', 'marks a pivotal moment', 'plays a vital/crucial role', 'reflects a broader', 'a rich heritage'. State what happened; let it carry its own weight. (Overlaps the closer rule: end at the last real point.)
- No hollow analysis via trailing '-ing' clauses ('..., highlighting its importance', '..., underscoring the need', '..., contributing to'). Cut the clause or make it a plain sentence with a subject.
- No vague attribution / weasel sourcing ('researchers say', 'some critics', 'industry reports', 'observers note', 'it is widely regarded'). Name the person or source, or drop the claim. (See the Attribution cluster and 'never invent proper nouns'.)
- No 'it's important to note', 'it's worth noting', 'it should be mentioned' scaffolding; state the point directly. (Overlaps the openers rule on value-performing meta-commentary.)
- No knowledge-cutoff or self-referential disclaimers ('as of my last update', 'as an AI').

**Sentence shape**

- No negative parallelism in any form: 'not only X but also Y', 'not X, but Y', 'X rather than Y' as a rhetorical flourish. State it as a straight list or a plain clause. (Full rule in 'Framing people & judgement'.)
- No rule-of-three padding (three adjectives or three parallel phrases in a row purely for rhythm). One precise word beats three vague ones.
- No elegant variation: don't cycle through synonyms to avoid repeating a word ('constraints' … 'confines' … 'impediments'). Repeat the plain word.
- No copula-avoidance: 'is/are' beats dressed-up substitutes ('serves as', 'functions as', 'stands as') where a plain verb fits.
- No high-density AI vocabulary: 'delve', 'moreover', 'furthermore', 'additionally', 'intricate', 'pivotal', 'underscore', 'garner', 'bolster', 'leverage', 'landscape', 'realm', 'testament', 'crucial'. Prefer the everyday word.

**Structure & formatting**

- No formulaic conclusion sections ('In conclusion', 'Overall', 'Future outlook', 'Despite its X, it faces challenges'). (Overlaps the closer rule.)
- No inline-header vertical lists (bulleted items each opening with a **bolded label**: then a description) as a default; use prose, and don't bold within bullets. (Full rule in 'Prose vs bullets'.)
- No title case in headings; sentence case.
- No mechanical boldface on every key term. Bold sparingly, for genuine subheaders.
- No em dashes (already the top rule), no curly/smart quotes (use straight), no stray horizontal rules before headings.
- No decorative emoji as structure. Slack marker emoji per the Emoji rule are fine; scattered ✨/🚀/✅ in prose are not.

**Collaborative-chatbot register**

- No assistant filler: 'I appreciate your feedback', 'Let me clarify', 'I hope this helps', 'Certainly!', 'Great question'. (Overlaps the validation-openers rule.)
- No 'such as' implying a fuller list that the source doesn't support; give the actual examples or say it's illustrative ('e.g.').
