...
description: "generate me&u Engineering On Call report"
---
...
- **Recipient source:** prefer the most recent _sent_ Gmail examples for current recipients
...
3. **Trigger the workflow** on `main`:
   ...
   ```
   The workflow takes no inputs (`workflow_dispatch` only) and defaults to the previous month
   via `full-report.sh`.
4. **Wait for the run to complete.** Find the run and watch it:
   ...
   gh run watch <run-id> --repo mr-yum/pagerduty-on-call --exit-status
   ```
...
7. **Build the `.xlsx` attachment** named `oncall-<month>-<year>.xlsx` with one sheet named
   `oncall-<month>-<year>`. Write it to `outputs/` unless the requester specifies a path.
...
## How the hours are bucketed (no double-counting)

The report buckets every on-call hour into exactly **one** category via `bucketFor` in
`mr-yum/pagerduty-on-call`, using strict precedence: **Holiday → Weekend → Office(excluded)
→ Weekday**. `total` is incremented for every non-office hour, so
...
Use Python with `openpyxl` in a throwaway venv (system Python is PEP-668 externally-managed,
so `pip install` needs a venv or `--break-system-packages`). Keep headers visibly styled
(bold, on a filled cell), never white text on a transparent/blank fill.
...
- **Do not attach the `.xlsx` to the draft.** Leave the `[attach Excel version of the table]`
  ...
  (Base64-encoding the attachment into the draft is slow and Paul prefers to attach it and the
  screenshot himself.)
- Keep the body plain; do not reproduce the user's signature, formatting, images, or social
  links.
- If a draft with an old body/attachment already exists, create a replacement draft and
  clearly tell the requester which draft is the newest one.
