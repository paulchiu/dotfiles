...
- Australian spelling.
- Backticks for code identifiers, filenames, config keys, and CLI commands; preserve on-disk casing (`CLAUDE.md` not `claude.md`, `package-lock.json`, `npm typecheck`).
...
- **Implementation guidance** is the section that most often decides whether a card runs on the first pass. See `### Writing the implementation guidance section` below for the full rules; the short version is: link to specific files and symbols, show problematic code as a snippet, and order the bullets primary → alternatives → constraints → uncertainty.
...
The implementation guidance section is what makes a card executable. Research the codebase as part of drafting, don't wait to be asked. When you have access to the repo (locally or via GitHub search), read it. If you do not have access, say so explicitly in the draft and flag the section as best-effort so the user can fill in file references themselves.
...
1. **Primary approach** — name the most likely entry point by symbol and file, with a GitHub deep link to the exact line. Frame it as the 'culprit' or 'entry point' rather than as a prescription; the agent still owns the final call. Default to reproducing the referenced code as a fenced snippet whenever it helps illustrate what needs to change; a reader shouldn't have to click through to the link to see the edit site. Skip the snippet only when the code is too long (>~10 lines) or purely structural (e.g. pointing at a module boundary, not a line).
2. **Alternatives** — patterns elsewhere in the repo the agent can mirror, or a fallback approach if the primary one does not fit. Label multiple attempts as 'attempt 1', 'attempt 2' if ordering matters.
3. **Constraints** — binding rules the agent must respect (do not introduce dependency X, must stay inside the existing transaction, must reuse `src/utils/foo.ts`).
4. **When uncertain** — a one-line instruction to stop and surface ambiguity rather than guessing.

**Formatting rules**

- Symbol links follow APA/Harvard-style citation: `` `Class.method` ([GitHub](https://github.com/org/repo/blob/main/path/to/file.ext#L123)) ``. Symbol in backticks, then a parenthetical `[GitHub]` label hyperlinked to the exact line. Use `#L123-L145` for line ranges. Do not wrap the backticked symbol inside the link text.
- Backtick all code identifiers, filenames, config keys, and CLI commands; preserve on-disk casing (`CLAUDE.md`, `package.json`, `npm typecheck`).
- Use fenced code blocks tagged by language (`typescript, `python, ```sql) when showing problematic or reference code. Keep snippets short; if they run longer than ~10 lines, link to the file instead.
- Mark edit sites inside snippets with emoji comment arrows so the agent's eye lands on the right line:
  - `// 👇 ...` on the line above when the target line is long or the comment would push it past a reasonable width.
  - `// 👈 ...` as a trailing inline comment when the target line is short enough to fit one.
  - Choose one per site; don't stack both. The comment text after the arrow should name the edit (e.g. `// 👈 missing tax recalc here`), not restate the code.
...
- The heading in the Linear card is `## Implementation guidance` (sentence case). Do not capitalise the `g`.

**Worked example**

Rendered output for a small refund bug:

````markdown
## Implementation guidance

- The culprit is most likely `OrderService.finalise` ([GitHub](https://github.com/org/repo/blob/main/src/orders/order.service.ts#L87)), which short-circuits before the tax recalc on refund:

  ```typescript
  // 👇 early-return skips the tax recalc below
  if (order.status === "refunded") {
    return; // 👈 missing tax adjustment here
  }
  ```

- Alternative: mirror `InvoiceService.finalise` ([GitHub](https://github.com/org/repo/blob/main/src/invoices/invoice.service.ts#L42)), which routes refunds through the shared adjustment pipeline.
- Constraints: must stay inside the existing `@Transactional()` boundary; do not introduce a new dependency; reuse `src/tax/recalc.ts` rather than inlining the maths.
- When uncertain: stop and propose a short plan rather than guessing.
````
...
2. **Research the codebase** to fill in implementation guidance. Default to doing this whenever you have repo access (current working directory, a repo the user has opened, or GitHub search); do not wait for the user to prompt you. Follow the research checklist in `### Writing the implementation guidance section` so the draft lands with real file paths, symbol links, and code snippets. If you have no repo access, state that explicitly and mark the section best-effort.
...
4. **Ask clarifying questions** before rewriting. Good questions cover:
   ...
   - Risk tier
5. **Research the codebase** to fill in file paths, patterns, and verification commands. Default to doing this whenever you have repo access; do not wait for the user to prompt you. Follow the research checklist in `### Writing the implementation guidance section`. If you have no repo access, state that explicitly and mark the section best-effort.
